 ...
 * Works with any provider the user has configured (BYOB).
 */
export function GenerationView() {
  ...
  const { generateAllPanels, cancelGeneration, generationStatus } = useGeneration();

  const isConnected = providerStatus.connected;
  const canGenerate = panels.length > 0 && !isGenerating && isConnected;
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-center text-[10px] text-manga-gray-400">
        {isConnected
          ? `Using ${PROVIDER_META[providerConfig.type].label}. Images are generated by your own backend.`
          : 'Configure and connect a provider above to start generating.'}
      </p>
    </div>
  );
}
